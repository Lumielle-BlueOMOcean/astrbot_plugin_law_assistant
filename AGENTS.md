...
- The structured-material v1 validator is pure validation/preview; the separate 0.4B ingestion service owns hash-bound PDF+JSON prepare/confirm persistence. Both paths must preserve block order, locators and provenance, and neither may grant verified-real-question or official-case identity.
...
- Structured Material v1 imports must accept the original document and JSON as a hash-bound pair through `prepare -> preview -> confirm`; never route validated structured JSON through legacy `segment_document()`.
- Structured imports persist original/JSON sources, payload hash, shared materials, ordered content blocks, subquestions, locators and review candidates in schema v10. Imported questions remain `real_question_candidate` / `pending_review`; they must never become verified real questions or official cases automatically.
- Question Session schema v11 stores an immutable, hash-bound question snapshot per exact QQ `unified_msg_origin`; a new question supersedes only the open session in that same scope. Prompt/material display must not include answers or explanations; reveal each only after explicit `answer` / `explanation` action, and never ask an LLM to invent missing reference content.
- Manual question publication and daily question delivery share the same answer-free session renderer. Create a target-scoped session only after that target's send succeeds; keep the exact body previewed, and do not activate sessions for failed/unknown deliveries.
- `real_question_candidate` stays pending and is only available through explicit operator study; it must not enter verified-real selection or be relabeled as a true exam question. Preserve multi-material and subquestion answer mapping in snapshots.
- The original user PDF and generated full structured JSON/report are local acceptance artifacts only. Do not commit, embed, upload or copy their real question text into tracked tests; use synthetic fixtures for regression coverage.
